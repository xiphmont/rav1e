...
macro_rules! tile_restoration_plane_common {
  ...
  ($name:ident, $tru_type:ident $(,$opt_mut:tt)?) => {
    impl<'a> $name<'a> {
      ...
      pub fn restoration_unit_index(&self, sbo: SuperBlockOffset) -> Option<(usize, usize)> {
        // is this a stretch block?
        let x_stretch = sbo.x < self.rp_cfg.sb_cols &&
          sbo.x >> self.rp_cfg.sb_shift >= self.units.cols;
        let y_stretch = sbo.y < self.rp_cfg.sb_rows &&
          sbo.y >> self.rp_cfg.sb_shift >= self.units.rows;

        let x = (sbo.x >> self.rp_cfg.sb_shift) - if x_stretch {1}else{0};
        let y = (sbo.y >> self.rp_cfg.sb_shift) - if y_stretch {1}else{0};
      ...
      pub fn restoration_unit_countable(&self, sbo: SuperBlockOffset) -> usize {
      ...
      pub fn restoration_unit_last_sb<T: Pixel>(&self, fi: &FrameInvariants<T>,
                                            sbo: SuperBlockOffset) -> bool {
        // there is 1 restoration unit for (1 << sb_shift) super-blocks
        let mask = (1 << self.rp_cfg.sb_shift) - 1;
        // is this a stretch block?
        let x_stretch = sbo.x >> self.rp_cfg.sb_shift >= self.units.cols;
        let y_stretch = sbo.y >> self.rp_cfg.sb_shift >= self.units.rows;

        let last_x = (sbo.x & mask == mask && !x_stretch) || sbo.x == fi.sb_width-1;
        let last_y = (sbo.y & mask == mask && !y_stretch) || sbo.y == fi.sb_height-1;
...
macro_rules! tile_restoration_state_common {
  ...
  ($name:ident, $trp_type:ident, $iter:ident $(,$opt_mut:tt)?) => {
    impl<'a> $name<'a> {
      ...
      #[inline(always)]
      pub fn has_restoration_unit(&self, sbo: SuperBlockOffset, pli: usize) -> bool {
