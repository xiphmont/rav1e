...
fn sgrproj_box_sum_slow(a: &mut i32, b: &mut i32,
                        stripe_y: isize, stripe_h: usize,
                        x: isize, y: isize,
                        r: usize, n: i32, one_over_n: i32, s: i32, bdm8: usize,
                        backing: &PlaneSlice, backing_w: usize, backing_h: usize,
                        cdeffed: &PlaneSlice, cdeffed_w: usize, cdeffed_h: usize) {  
  let mut ssq:i32 = 0;
  let mut sum:i32 = 0;
  ...
  for yi in y-r as isize..=y+r as isize {
    ...
    for _xi in 0..left {
      let c = p[(r as isize - x) as usize] as i32;
      ssq += c*c;
      sum += c;
    }
...
fn sgrproj_box_sum_fastxy_r1(a: &mut i32, b: &mut i32, x: isize, y: isize,
                             s: i32, bdm8: usize, p: &PlaneSlice) {
  let mut ssq:i32 = 0;
  let mut sum:i32 = 0;
  for yi in -1..=1 {
    let x = p.reslice(x - 1, y + yi).as_slice();
    ssq += x[0] as i32 * x[0] as i32 +
      x[1] as i32 * x[1] as i32 +
      x[2] as i32 * x[2] as i32;
    sum += x[0] as i32 + x[1] as i32 + x[2] as i32;
  }
...
fn sgrproj_box_sum_fastxy_r2(a: &mut i32, b: &mut i32, x: isize, y: isize,
                             s: i32, bdm8: usize, p: &PlaneSlice) {
  let mut ssq:i32 = 0;
  let mut sum:i32 = 0;
  for yi in -2..=2 {
    ...
    ssq += x[0] as i32 * x[0] as i32 +
      x[1] as i32 * x[1] as i32 +
      x[2] as i32 * x[2] as i32 +
      x[3] as i32 * x[3] as i32 +
      x[4] as i32 * x[4] as i32;
    sum += x[0] as i32 + x[1] as i32 + x[2] as i32 + x[3] as i32 + x[4] as i32;
  }
...
fn sgrproj_box_sum_fastx_r1(a: &mut i32, b: &mut i32,
                            stripe_y: isize, stripe_h: usize,
                            x: isize, y: isize,
                            s: i32, bdm8: usize,
                            backing: &PlaneSlice, backing_h: usize,
                            cdeffed: &PlaneSlice, cdeffed_h: usize) {  
  let mut ssq:i32 = 0;
  let mut sum:i32 = 0;
  for yi in y-1..=y+1 {
    let src_plane;
...
fn sgrproj_box_sum_fastx_r2(a: &mut i32, b: &mut i32,
                            stripe_y: isize, stripe_h: usize,
                            x: isize, y: isize,
                            s: i32, bdm8: usize,
                            backing: &PlaneSlice, backing_h: usize,
...
fn sgrproj_box_ab_r1(af: &mut[i32; 64+2],
                     bf: &mut[i32; 64+2],
...
fn sgrproj_box_ab_r2(af: &mut[i32; 64+2],
                     bf: &mut[i32; 64+2],
                     stripe_x: isize, stripe_y: isize, stripe_h: usize,
                     s: i32, bdm8: usize,
                     backing: &PlaneSlice, backing_w: usize, backing_h: usize,
...
fn sgrproj_box_f_r0(f: &mut[i32; 64], x: usize, y: isize, h: usize, cdeffed: &PlaneSlice) {
  for i in cmp::max(0, -y) as usize..h {
    f[i as usize] = (cdeffed.p(x, (y + i as isize) as usize) as i32) << SGRPROJ_RST_BITS;
  }
}

fn sgrproj_box_f_r1(af: &[&[i32; 64+2]; 3], bf: &[&[i32; 64+2]; 3], f: &mut[i32; 64],
                 x: usize, y: isize, h: usize, cdeffed: &PlaneSlice) {
...
fn sgrproj_box_f_r2(af: &[&[i32; 64+2]; 3], bf: &[&[i32; 64+2]; 3], f: &mut[i32; 64],
                    x: usize, y: isize, h: usize, cdeffed: &PlaneSlice) {
  let shift = 5 + SGRPROJ_SGR_BITS - SGRPROJ_RST_BITS;
  ...
  for i in (cmp::max(0, -y) as usize..h).step_by(2) {
    ...
    let bo =
      5 * (bf[0][i+2] + bf[2][i+2]) + 
      6 * (bf[1][i+2]);
    let v = (a + ao) * cdeffed.p(x, (y+i as isize) as usize) as i32 + b + bo;
    f[i as usize] = v + (1 << shift >> 1) >> shift;
    let vo = ao * cdeffed.p(x, (y + i as isize) as usize + 1) as i32 + bo;
    f[i as usize + 1] = vo + (1 << shifto >> 1) >> shifto;
  }
}

pub fn sgrproj_stripe_filter(set: u8, xqd: [i8; 2], fi: &FrameInvariants,
                             crop_w: usize, crop_h: usize,
                             stripe_w: usize, stripe_h: usize,
                             cdeffed: &PlaneSlice, deblocked: &PlaneSlice, out: &mut PlaneMutSlice) {
...
pub fn sgrproj_solve(set: u8, fi: &FrameInvariants,
                     input: &PlaneSlice,
                     cdeffed: &PlaneSlice, cdef_w: usize, cdef_h: usize) -> (i8, i8) {
  ...
  for xi in 0..cdef_w {
    ...
    for yi in 0..cdef_h {
      let u = (cdeffed.p(yi,xi) as i32) << SGRPROJ_RST_BITS;
      let s = (input.p(yi,xi) as i32) << SGRPROJ_RST_BITS;
      let f2 = f_r2[yi] - u;
      let f1 = f_r1[yi] - u;
...
fn wiener_stripe_filter(coeffs: [[i8; 3]; 2], fi: &FrameInvariants,
                      crop_w: usize, crop_h: usize,
                      stripe_w: usize, stripe_h: usize,
