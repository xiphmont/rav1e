...
fn rdo_loop_plane_error(sbo: &SuperBlockOffset, fi: &FrameInvariants,
                        fs: &FrameState, bc: &BlockContext, test: &Frame, pli: usize) -> u64 {
...
pub fn rdo_loop_decision(sbo: &SuperBlockOffset, fi: &FrameInvariants,
                         fs: &mut FrameState,
          ...
          for pli in 0..3 {
            match best_lrf[pli] {
              RestorationFilter::None{} => {
                ...
                cost[pli] = err as f64 + get_lambda(fi) * rate as f64 / ((1<<OD_BITRES) as f64);
                cost_acc += cost[pli];
              }
    ...
    if fi.sequence.enable_restoration {
      ...
      for pli in 0..3 {
        for set in 0..16 {
          ...
          let cost = err as f64 + get_lambda(fi) * rate as f64 / ((1<<OD_BITRES) as f64);
